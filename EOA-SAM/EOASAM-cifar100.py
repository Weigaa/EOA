...
def getbest_params(model,mysrc):
    """ Gradient averaging. """
    for param in model.parameters():
        # dist.all_reduce(param.grad.data, op=dist.ReduceOp.SUM, group=0)
        dist.broadcast(param.data, src=mysrc)
...
def GWO(mytensorlist, bestlr, myrank, epoch, populationsize=4, decay = 1):
    ...
    dist1 = (dist1 / 4) * bestlr * decay
    dist2 = (dist2 / 4) * bestlr * decay
    beta[1] = abs(((bestlr-dist1)+(alpha[1]-dist2))/2)
    if myrank == beta[2]:
        return beta[1]
    #更新delta
    ...
    dist1 = (dist1 / 4) * bestlr * decay
    dist2 = (dist2 / 4) * bestlr * decay
    dist3 = (dist3 / 4) * bestlr * decay
    delta[1] = abs(((bestlr-dist1)+(alpha[1]-dist2)+(beta[1]-dist3))/3)
    if myrank == delta[2]:
        return delta[1]
    #更新omega
    for i in range(len(omega)):
        if myrank == omega[i][2]:
            ...
            dist2 = (dist2 / 4) * bestlr * decay
            dist3 = (dist3 / 4) * bestlr * decay
            omega[i][1] = abs(((alpha[1] - dist1) + (beta[1] - dist2)+(delta[1] - dist3)) / 3)
            return omega[i][1]
...
def main(rank):
    ...
    for epoch in range(args.epochs):
        ...
        mydecay = 0.2
        # mydecay = 1
        reference_lr = 0.8*(math.cos((epoch) * math.pi / 200) + 1) / 2
        newbaselr = (bestlr + reference_lr)/2
        if rank == 0:
            print("bestlr is",bestlr,"referencelr is",reference_lr,"newbaselr is",newbaselr)
        #GWO计算下一epoch学习率
        ...
        #WOA计算下一epoch学习率
            ...
            #mylr = abs(WOA(mylr=mylr, bestlr=newbaselr, epoch=epoch, mytensorlist=mytensor_list, populationsize = WOAsize, decay = mydecay))
        # #数据后处理，剔除偏差过大的数据
        # if mylr < newbaselr * 0.95:
        #     mylr = newbaselr * random.uniform(0.95,1)
        # elif mylr > newbaselr * 1.05:
        #     mylr = newbaselr * random.uniform(1,1.05)
